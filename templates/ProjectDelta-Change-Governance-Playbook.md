...
For a neutral, fillable version intended for another application, use
`application-change-governance-playbook.md`. Preserve this ProjectDelta document
as the historical source of the generalized rules and lessons.
...
For Phase 4 quality work, `change-request.md` remains the canonical request.
Use `lint-static-analysis-baseline.md` as its check-only companion when measuring
an existing lint/static-analysis control, or
`bounded-refactor-change-request.md` as its companion for a behavior-preserving
refactor. Do not create both companions by default. When linting is only
refactor validation, record it in the bounded-refactor companion; create a
separate lint-baseline request only when measuring the existing lint state is
...
| `lint-static-analysis-baseline.md` | 4.1 | Check-only profile for measuring an existing lint/static-analysis control without source edits |
| `bounded-refactor-change-request.md` | 4.1 | Refactor profile for explicit behavior invariants, edit boundaries, and regression evidence |
